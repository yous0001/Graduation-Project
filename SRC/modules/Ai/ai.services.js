...
import { uploadFileBuffer } from "../../utils/cloudinary.utils.js";
import {
  GoogleGenerativeAI,
  HarmCategory,
  HarmBlockThreshold,
  ...
  try {
    ...
    const response = await axios.post(
      aiConfig.endpoints.huggingFace.recipeGeneration,
      {
        inputs:
          ingredients +
          "note: give me most similar recipe without error please",
        parameters: {
          max_length: 50000,
          do_sample: true,
        },
...
export async function generateImage(prompt) {
  ...
  const data = {
    inputs:
      prompt +
      " note:please make it be looks delicious as much as possible , i need one image for the output recipe",
  };
...
export async function generateRecipeGemini(ingredients) {
  try {
    ...
    console.error(
      "Error generating recipe with Gemini:",
      error.message,
      error.response?.data || {}
    );
    throw error;
...
export async function parseMarkdownToJson(markdownText) {
  ...
  // Extract overview
  const overview = {};
  const overviewItems = root.querySelectorAll(
    'h2:contains("Overview") + ul li'
  );
  overviewItems.forEach((item) => {
    const [key, value] = item.textContent
      .split(": ")
      .map((s) => s.replace(/\*\*/g, "").trim());
    overview[key.toLowerCase().replace(" ", "")] = value;
  });
  overview.dietaryTags = overview.dietaryTags
    ? overview.dietaryTags.split(", ").map((tag) => tag.trim())
    : [];

  // Extract description
  const description =
    root.querySelector('h2:contains("Description") + p')?.textContent || "";

  // Extract ingredients
  const ingredients = root
    .querySelectorAll('h2:contains("Ingredients") + ul li')
    .map((item) => {
      ...
      return {
        ...
        substitute: match?.[3]?.includes("substitute")
          ? match[3].replace(/.*substitute:\s*/, "").trim()
          : undefined,
        notes:
          match?.[3] && !match[3].includes("substitute")
            ? match[3].trim()
            : undefined,
      };
    });
  ...
  const instructions = root
    .querySelectorAll('h2:contains("Instructions") + ol li')
    .map((item, index) => {
  ...
  // Extract tips
  const tipsAndVariations = root
    .querySelectorAll('h2:contains("Tips and Variations") + ul li')
    .map((item) => item.textContent.trim());

  // Extract nutrition
  const nutrition = {};
  const nutritionItems = root.querySelectorAll(
    'h2:contains("Nutritional Information") + ul li'
  );
...
export async function generateImageForGemini(recipeJson, retries = 2) {
  ...
  const apiKeys = Array.from(
    { length: aiConfig.limits.maxHuggingFaceKeys },
    (_, i) => process.env[`HUGGINGFACE_TOKEN${i + 1}`]
  ).filter(Boolean);
  let lastError = null;
  ...
  const prompt = `
        A beautifully plated dish of ${recipeJson.title.toLowerCase()}, showcasing ${recipeJson.description
    .split(".")[0]
    .toLowerCase()}. 
    ...
    for (let acceptIndex = 0; acceptIndex < retries; acceptIndex++) {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          const uploadResult = await uploadFileBuffer({
            buffer: imageBuffer,
            filename: `${recipeJson.title.replace(/\s+/g, "_").toLowerCase()}.${
              acceptTypes[acceptIndex].split("/")[1]
            }`,
            folder: `${process.env.UPLOADS_FOLDER}/chat-recipes`,
          });
          ...
          console.error(
            `❌ Attempt ${attempt + 1} with Accept: ${
              acceptTypes[acceptIndex]
            } and API key ${keyIndex + 1} failed:`,
            error.message
          );
          ...
          if (
            ...
            console.log(
              `Model may be sleeping or rate-limited dermal. Retrying after ${
                attempt + 1
              }s...`
            );
...
const moodKeywords = {
  ...
  inspired: {
    description:
      "artistic, sophisticated, and visually appealing to fuel creativity",
...
function analyzeMood(mood) {
  ...
  Object.keys(moodKeywords).forEach((key) => {
    if (normalizedMood.includes(key)) {
      ...
      style.cuisine =
        moodKeywords[key].cuisine !== "Any"
          ? moodKeywords[key].cuisine
          : style.cuisine;
      style.difficulty =
        moodKeywords[key].difficulty === "Easy" ? "Easy" : style.difficulty;
      style.dietaryTags.push(...moodKeywords[key].tags);
      style.suggestedIngredients.push(
        ...moodKeywords[key].suggestedIngredients
      );
    }
  });
  ...
  if (
    matchedMoods.includes("happy") &&
    (matchedMoods.includes("stressed") || matchedMoods.includes("pressures"))
  ) {
    style.description =
      "vibrant yet quick and soothing to balance joy and stress";
    style.difficulty = "Easy";
    style.dietaryTags = [
      ...new Set([...style.dietaryTags, "Quick", "Healthy"]),
    ];
  }
...
export async function generateRecipeByMood(mood) {
  ...
  const prompt = `
      Generate a recipe tailored to the user's mood: "${mood}". The dish should be ${
    style.description
  }, suitable for ${style.cuisine} cuisine, with a ${
    style.difficulty
  } difficulty level. Use ingredients like ${style.suggestedIngredients.join(
    ", "
  )} to align with the mood.
...
async function parseDietPlanMarkdown(markdownText) {
  ...
  for (const dayNode of dayNodes) {
    ...
    for (const mealNode of mealNodes) {
      const mealType = mealNode.textContent
        .toLowerCase()
        .replace(/\s+/g, "")
        .replace(/\(\d+kcal\)/, "");
      ...
      while (
        recipeNode &&
        recipeNode.tagName !== "H3" &&
        recipeNode.tagName !== "H2"
      ) {
      ...
      // Description
      const descriptionItem = sectionItems.find((item) =>
        item.textContent.toLowerCase().includes("description")
      );
      const description =
        descriptionItem
          ?.querySelector("p")
          ?.textContent.replace(/.*[Dd]escription.*?:\s*/, "")
          .trim() || "";

      // Ingredients
      const ingredientsItem = sectionItems.find((item) =>
        item.textContent.toLowerCase().includes("ingredients")
      );
      const ingredients = ingredientsItem?.querySelector("ul")
        ? Array.from(ingredientsItem.querySelectorAll("ul > li")).map(
            (item) => {
              const text = item.textContent.trim();
              const match = text.match(
                /^([\d\s/½¼¾⅓⅔⅛⅜⅝⅞]+)?\s*([a-zA-Z\s]+?)(?:\s*\((.*?)\))?$/
      ...
      // Instructions
      const instructionsItem = sectionItems.find((item) =>
        item.textContent.toLowerCase().includes("instructions")
      );
      const instructions = instructionsItem?.querySelector("ol")
        ? Array.from(instructionsItem.querySelectorAll("ol > li")).map(
            (item, index) => {
              const text = item.textContent.trim();
      ...
      const nutritionItem = sectionItems.find((item) =>
        item.textContent.toLowerCase().includes("nutritional information")
      );
      const nutrition = {};
      ...
      // Dietary Tags
      const tagsItem = sectionItems.find((item) =>
        item.textContent.toLowerCase().includes("dietary tags")
      );
      const dietaryTags =
        tagsItem
          ?.querySelector("p")
          ?.textContent.split(",")
          .map((tag) => tag.trim())
          .filter((tag) => tag && !tag.includes("Note:")) || [];

      // Log parsed meal for debugging
      console.log(`Parsed meal ${mealType} for Day ${dayNumber}:`, {
        title,
        description,
        ingredients,
        instructions,
...
export async function generateDietPlan({
  ...
  if (
    ...
    throw new Error(
      "All fields (height, age, weight, fatPercentage, goal, preferences, gender) are required"
    );
  }
  ...
  const calorieSplit = {
    ...
    lunch: Math.round(dailyCalories * 0.3),
  ...
  for (let day = 1; day <= 30; day++) {
    console.log(
      `Generating diet plan for Day ${day} with GEMINI_API_KEY${currentKeyIndex}...`
    );
    ...
    const prompt = `
      ...
      Provide 5 recipes for the day (breakfast, lunch, dinner, two snacks) with a total daily calorie intake of approximately ${Math.round(
        dailyCalories
      )} kcal, distributed as:
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        console.log(
          `Markdown for Day ${day}:`,
          markdownText.substring(0, 1000)
        );
        ...
        Object.entries(parsedPlan.days[0].meals).forEach(
          ([mealType, recipe]) => {
            if (
              recipe.instructions.length <
              validationConfig.arrays.recipeInstructions.min
            ) {
              console.warn(
                `Warning: ${mealType} for Day ${day} has only ${recipe.instructions.length} instructions`
              );
            }
          }
        );
        ...
        console.error(
          `Attempt ${
            attempt + 1
          } for Day ${day} with GEMINI_API_KEY${currentKeyIndex} failed:`,
          error.message
        );
        ...
        if (
          ...
          if (currentKeyIndex < maxKeys) {
            ...
            if (!apiKey) {
              console.error(
                `No more API keys available after GEMINI_API_KEY${
                  currentKeyIndex - 1
                }`
              );
              break;
            ...
            console.error(
              `All API keys (up to GEMINI_API_KEY${maxKeys}) exhausted`
            );
          }
        }
        ...
        if (attempt === 2) {
          console.error(
            `Failed to generate Day ${day} after 3 attempts. Using placeholder.`
          );
  ...
  return {
    ...
    meals: {
      breakfast: {
        ...
        instructions: [
          { step: 1, action: "Prepare", description: "Prepare the food item." },
        ],
        nutrition: {
          calories: `${calorieSplit.breakfast} kcal`,
          protein: "Unknown",
          fat: "Unknown",
      ...
      lunch: {
        ...
        nutrition: {
          calories: `${calorieSplit.lunch} kcal`,
          protein: "Unknown",
          fat: "Unknown",
          carbohydrates: "Unknown",
        },
      ...
      dinner: {
        ...
        nutrition: {
          calories: `${calorieSplit.dinner} kcal`,
          protein: "Unknown",
          fat: "Unknown",
          carbohydrates: "Unknown",
        },
        ...
        instructions: [
          {
            step: 1,
            action: "Prepare",
            description: "Prepare the snack item.",
          },
        ],
        nutrition: {
          calories: `${calorieSplit.snack1} kcal`,
          protein: "Unknown",
          fat: "Unknown",
          carbohydrates: "Unknown",
        },
      ...
      snack2: {
        ...
        nutrition: {
          calories: `${calorieSplit.snack2} kcal`,
          protein: "Unknown",
          fat: "Unknown",
          carbohydrates: "Unknown",
        },
...
export async function enhanceRecipesGemini(recommendations) {
  try {
    ...
    console.error(
      "Error enhancing recipes with Gemini:",
      error.message,
      error.response?.data || {}
    );
    throw error;
