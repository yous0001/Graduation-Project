...
import {
  enhanceRecipesGemini,
  generateDietPlan,
  generateImage,
  generateImageForGemini,
  generateRecipeByMood,
  generateRecipeGemini,
  generateRecipeM1,
} from "./ai.services.js";
...
export const getLegacyRecommendation = async (req, res) => {
  try {
    const { ingredients } = req.body;
...
export const getRecipeByMood = async (req, res) => {
  try {
    const { mood } = req.body;
...
export const getDietPlan = async (req, res, next) => {
  try {
    ...
    if (!Object.values(validGoals).includes(normalizedGoal)) {
      return res.status(400).json({
        success: false,
        error: `Invalid goal. Must be one of: ${Object.values(validGoals).join(
          ", "
        )}`,
      });
    }
    ...
    try {
      ...
      pdfBuffer = await generateDietPlanPdf(
        dietPlan,
        30,
        preferences,
        userData
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to generate diet plan" });
    next(error);
  }
};
...
export const searchWithAi = async (req, res) => {
  try {
    ...
    if (!ingredients) {
      return res
        .status(400)
        .json({ success: false, message: "Ingredients is required" });
    }

    // Call Flask API to get initial recommendations
    const { data: response } = await axios.post(
      aiConfig.endpoints.flask.recommend,
      {
        ingredients,
        start,
        count,
    ...
    console.log(
      chalk.bgYellow(`${newRecommendations.length} new recipes to be enhanced.`)
    );
    ...
    if (newRecommendations.length > 0) {
      ...
      for (const recipe of enhancedRecipes) {
        try {
          //console.log(recipe);
          const imageUrl = await generateImageForGemini(recipe.recipeJson);
          ...
          const recommendation = new Recommendation({
            ...
            image: recipe.image,
            originalIngredients:
              recommendations.find((r) => r.title === recipe.originalTitle)
                ?.ingredients || [],
            category,
          });
          ...
          console.error(
            `Failed to generate image or save recipe ${recipe.originalTitle}:`,
            error.message
          );
          recipe.image = {
            imageUrl: "https://via.placeholder.com/800x400?text=Recipe+Image",
          };
          const recommendation = new Recommendation({
            originalTitle: recipe.originalTitle,
            recipeJson: recipe.recipeJson,
