...
export const addMealDBCountries = async (req, res) => {
  const user = req.user;
  const response = await axios.get(
    `${apiConfig.mealDB.baseUrl}${apiConfig.mealDB.endpoints.areasList}`
  );
  ...
  if (insertedCountries.length === 0) {
    return res
      .status(200)
      .json({ success: true, message: "No new countries to be added" });
  }
...
export const updateCountry = async (req, res, next) => {
  ...
  if (!id) {
    return next(new Error("Please provide the country id", { cause: 400 }));
  }
  if (!name && !description) {
    return next(
      new Error("Please provide the country name or description", {
        cause: 400,
      })
    );
  }
  ...
  res
    .status(200)
    .json({ success: true, message: "Country updated successfully", country });
};
