...
export const addToCart=async(req, res, next) => {
    ...
    if(!ingredient){
        return res.status(404).json({ success: false, message: 'Ingredient not available' });
    }
    ...
    if(!cart){
        ...
        return res.status(200).json({ success: true, message:"add to cart successfully", cart:newCart})
    }

    const isIngredeintInCart=cart.ingredients.find(ingred=>ingred.IngredientID==ingredientId)
    if(isIngredeintInCart){
        return res.status(400).json({ success: false, message: 'Ingredient already in cart' });
    }
    ...
    return res.status(200).json({ success: true, message:"add to cart successfully", cart})
}
...
export const getCart=async(req,res,next)=>{
    ...
    return res.status(200).json({ success: true, message:"get cart successfully" ,cart });
}
...
export const removeFromCart=async(req,res,next)=>{
    ...
    if(!cart){
        return res.status(404).json({ success: false, message: 'ingredient not found in cart' });
    }
    ...
    await cart.save()
    res.status(200).json({ success: true, message:"removed successfully",cart})
}
export const updateCart=async(req,res,next)=>{
    ...
    const ingredient=await Ingredient.findOne({_id:ingredientId, stock:{$gte:quantity}})
    if(!ingredient){
        return res.status(400).json({ success: false, message: 'Ingredient not available' });
    }
    ...
    await cart.save()
    res.status(200).json({ success: true, message:"updated successfully",cart})
}
