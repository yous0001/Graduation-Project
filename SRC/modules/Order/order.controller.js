...
export const createOrderByCart = async (req, res, _next) => {
  const userId = req.user._id;
  const { shippingAddressID, contactNumber, couponCode, paymentMethod } =
    req.body;
  let couponId = null;
  ...
  const isNotAvaliable = cart.ingredients.find(
    (ing) => ing.IngredientID.stock < ing.quantity
  );
  if (isNotAvaliable) {
    return res
      .status(400)
      .json({
        success: false,
        message: `ingredient ${isNotAvaliable.IngredientID.name} is not available`,
      });
  }
  ...
  if (isAddrecessExist.userId.toString() !== userId.toString()) {
    return res
      .status(400)
      .json({ success: false, message: "this is not one of your addresses." });
  }
  ...
  if (couponCode) {
    const { updatedTotal, couponId: appliedCouponId } =
      await applyCouponDiscount(couponCode, userId, total);
    total = updatedTotal;
...
export const payWithStripe = async (req, res, next) => {
  ...
  if (!order){
    return next(new Error("you don't have this order", { cause: 404 }));}
  if (order.paymentMethod === orderStatuses.cancelled){
    return next(new Error("order have been canceled", { cause: 400 }));}
  if (order.orderStatus !== orderStatuses.pending){
    return next(
      new Error("order is not pending to be paid or have already been paid", {
        cause: 400,
      })
    );}
  if (order.paymentMethod === paymentMethods.cash){
    return next(
      new Error("order is checked to be paid with cash", { cause: 400 })
    );}
  const paymentObject = {
...
export const orderOverview = async (req, res) => {
  ...
  if (couponCode) {
    const {
      ...
      couponId: appliedCouponId,
      discount,
    } = await checkCouponDiscount(couponCode, userId, total);
    total = updatedTotal;
    couponId = appliedCouponId;
...
export const cancelOrder = async (req, res, next) => {
  ...
  const order = await Order.findOneAndUpdate(
    { _id: id },
    { orderStatus: orderStatuses.cancelled, updatedBy: userId },
    { new: true }
  );
  if (!order) return next(new Error("order not found", { cause: 404 }));
  return res
    .status(200)
    .json({ success: true, message: "order canceled", order });
};
...
export const getSpecificOrder = async (req, res, next) => {
  ...
  if (order.userId.toString() !== req.user._id.toString())
    {return next(new Error("this is not one of your orders", { cause: 400 }));}
