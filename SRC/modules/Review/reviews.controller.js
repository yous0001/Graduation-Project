...
export const addReview = async (req, res) => {
  ...
  if (recipeId && ingredientId) {
    return res.status(400).json({
      success: false,
      message: "Review can be for either recipe or ingredient, not both.",
    });
  }
  ...
  let populatedReview = await Review.findById(newReview._id).populate({
    path: "userID",
    select: "username email profileImage.secure_url",
  });
  //average*numberOfRating=>totalreviews
  //totalReviews+rate/(numberOfReviews+1)=>to get new average
  targetDoc.Average_rating =
    (targetDoc.Average_rating * targetDoc.number_of_ratings + rate) /
...
export const deleteReview = async (req, res) => {
  ...
  if (!targetDoc) {
    return res
      .status(404)
      .json({ success: false, message: "Target item not found" });
  }
  //average*numberOfRating=>totalreviews
...
export const getReviews = async (req, res) => {
  ...
  if (!recipeId && !ingredientId) {
    return res.status(400).json({
      success: false,
      message: "Please provide either recipeId or ingredientId",
    });
  }
  ...
  if (recipeId && ingredientId) {
    return res.status(400).json({
      success: false,
      message: "Please provide only one of recipeId or ingredientId, not both",
    });
  }
  ...
  const reviewsWithCounts = reviews.map((review) => {
    ...
    const hasLiked =
      review.likes?.some((id) => id.toString() === userIdStr) || false;
    const hasDisliked =
      review.dislikes?.some((id) => id.toString() === userIdStr) || false;
    return {
      ...review.toObject(),
  ...
  res
    .status(200)
    .json({ success: true, reviews: reviewsWithCounts, user: user._id });
};
