...
const estimateProteinIntake = (dietPlan, weight, preferences) => {
  ...
  const estimatedProtein = Object.values(proteinPerMeal).reduce(
    (sum, val) => sum + val,
    0
  );
  const targetProteinPerKg = preferences.toLowerCase().includes("high protein")
    ? 2.0
    : 1.2;
...
const predictEffect = (dietPlan, preferences, userData = {}) => {
  ...
  if (normalizedGoal === "muscle gain") {
    ...
    const rate =
      muscleGainRates[trainingExperience] || muscleGainRates.beginner;
    const muscleEfficiency = proteinGrams >= 1.6 * weight ? 0.7 : 0.5;
    const muscleGainKg = Math.min(
      (totalSurplus * muscleEfficiency) / 2500,
      rate.max * (30 / 30.42)
    ).toFixed(1);
    const muscleGainMin = Math.max(
      muscleGainKg * aiConfig.pdf.calculations.muscleGainMultiplier,
      rate.min *
        (aiConfig.pdf.calculations.daysInMonth /
          aiConfig.pdf.calculations.daysInMonthAdjusted)
    ).toFixed(1);
    const fatLossKg = Math.min(
      (proteinGrams * aiConfig.pdf.calculations.daysInMonth * 0.01) /
        aiConfig.pdf.calculations.caloriesPerKgFat,
      aiConfig.pdf.calculations.maxFatLossDaily
    ).toFixed(1);
    const fatLossMax = (
      fatLossKg * aiConfig.pdf.calculations.fatLossMultiplierHigh
    ).toFixed(1);
    const strengthGain = trainingExperience === "beginner" ? "5–10%" : "2–5%";
    const waistReduction = fatLossKg > 0 ? "1–2cm" : "0–1cm";

    return `With your high-protein diet (~${Math.round(
      proteinGrams
    )}g/day) and muscle gain goal, you can expect to gain ${muscleGainMin}–${muscleGainKg}kg of muscle, lose ${fatLossKg}–${fatLossMax}kg of fat, increase strength by ${strengthGain}, and reduce waist circumference by ${waistReduction} over 30 days, assuming consistent resistance training 3–4 times per week and 85–90% diet adherence.`;
    ...
    const fatLossKg = Math.min(
      totalDeficit / aiConfig.pdf.calculations.caloriesPerKgFat,
      aiConfig.pdf.calculations.maxFatLossKg
    ).toFixed(1);
    const fatLossMax = (
      fatLossKg * aiConfig.pdf.calculations.fatLossMultiplierLow
    ).toFixed(1);
    ...
    return `With your diet (~${Math.round(
      proteinGrams
    )}g/day) and weight loss goal, you can expect to lose ${fatLossKg}–${fatLossMax}kg of fat, preserve muscle with ${muscleLoss} loss, maintain or increase strength by ${strengthGain}, and reduce waist circumference by ${waistReduction} over 30 days with regular exercise and 85–90% diet adherence.`;
    ...
    const waistChange =
      proteinGrams >= 1.4 * weight ? "0–1cm reduction" : "stable";

    return `With your diet (~${Math.round(
      proteinGrams
    )}g/day) and maintenance goal, you can expect to ${bodyCompChange} body composition, increase strength by ${strengthGain}, and maintain or slightly reduce waist circumference (${waistChange}) over 30 days with consistent exercise and 85–90% diet adherence.`;
  }

  return `With your diet (~${Math.round(
    proteinGrams
  )}g/day), you can expect improved health, energy, and body composition over 30 days with consistent exercise and 85–90% diet adherence.`;
};

export async function generateDietPlanPdf(
  dietPlan,
  maxDays = aiConfig.limits.maxDaysInPdf,
  preferences = "",
  userData = {}
) {
  try {
    ...
    if (!dietPlan?.dietPlan?.length || !dietPlan.totalCalories) {
      throw new Error(
        "Invalid diet plan: missing dietPlan array or totalCalories"
      );
    }
    ...
    try {
      imageBuffer = fs.readFileSync(
        "C:/Users/TW/One Drive/Desktop/graduation project/public/banner.png"
      );
    ...
    const addWrappedText = (text, x, y, options = {}) => {
      const {
        font = fonts.regular,
        size = aiConfig.pdf.layout.fontSize,
        color = colors.text,
        align = "left",
        maxWidth = doc.page.width - aiConfig.pdf.layout.pageMargin,
      } = options;
      let currentY = y;
      ...
      // Render text
      doc.fillColor(color).text(text, x, currentY, {
        align,
        width: maxWidth,
        lineBreak: true,
        lineGap: 3,
    ...
    if (imageBuffer) {
      ...
      doc.fillColor("white").rect(0, 0, doc.page.width, 80).fill(colors.header);
    }
    doc
      .font(fonts.bold)
      .fontSize(20)
      .fillColor("white")
      .text("Your Personalized Diet Plan", 0, imageBuffer ? 30 : 30, {
        align: "center",
      });
    ...
    yPos += 20;
    const tags =
      dietPlan.dietPlan[0]?.meals?.breakfast?.dietaryTags?.join(", ") ||
      preferences ||
      "Custom";
    yPos = addWrappedText(
      `This diet plan is tailored to your goals: ${escapeText(
        tags
      )}. Total daily calories: ${
        dietPlan.totalCalories
      } kcal. Enjoy your journey to better health!`,
    ...
    ["Breakfast", "Lunch", "Dinner", "Snack 1", "Snack 2"].forEach((meal) => {
      const kcal =
        calorieSplit[meal.toLowerCase().replace(" ", "")] || "Unknown";
      yPos = addWrappedText(
        `${meal}: ${kcal} kcal`,
        doc.page.margins.left,
        yPos,
    ...
    // Predicted Effect
    yPos += 10;
    yPos = addWrappedText("Predicted Effect", doc.page.margins.left, yPos, {
      font: fonts.bold,
      size: 12,
      color: colors.header,
      align: "center",
    ...
    yPos = addWrappedText(effectText, doc.page.margins.left, yPos, {
      font: fonts.regular,
      size: 10,
      color: colors.text,
      align: "center",
    ...
    dietPlan.dietPlan.slice(0, maxDays).forEach((day) => {
      ...
      doc
        ...
        .text(`Goal: ${dietPlan.goal || "Custom"}`, doc.page.width - 150, 20, {
          align: "right",
          width: 120,
        });
      ...
      Object.entries(day.meals).forEach(([mealType, recipe]) => {
        ...
        ["Calories", "Protein", "Fat", "Carbohydrates"].forEach((nutrient) => {
          currentY = addWrappedText(
            `• ${nutrient}: ${escapeText(
              nutrition[nutrient.toLowerCase()] || "Unknown"
            )}`,
            doc.page.margins.left + 10,
            currentY,
            { font: fonts.regular, size: 10, color: colors.text }
        ...
        currentY += 5;
        const tags = recipe.dietaryTags?.length
          ? recipe.dietaryTags
          : [preferences || "None"];
        tags.forEach((tag) => {
          currentY = addWrappedText(
