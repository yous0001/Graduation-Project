    ...
    for (let i = 0; i < retries; i++) {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            try {
                ...
                console.error(`❌ Attempt ${attempt + 1} with Accept: ${acceptTypes[i]} failed:`, error.message);
                lastError = error;
                ...
                if (
                    ...
                    console.log(`Model may be sleeping or rate-limited. Retrying after ${attempt + 1}s...`);
        ...
        // Log if all retries for this accept type failed
        if (i === retries - 1) {
            console.error('All retries for all accept types failed.');
        }
    }
