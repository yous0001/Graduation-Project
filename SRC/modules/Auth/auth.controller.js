...
import {
  forgetPasswordRequestEmailTemplete,
  loginVerificationEmailTemplete,
  resetPasswordSuccess,
  verificationEmailTemplate,
} from "./../Services/emailTempletes.js";
...
export const register = async (req, res, _next) => {
  ...
  const isEmailsent = await sendmailservice({
    ...
    message: verificationEmailTemplate.replace(
      "{{url}}",
      `${process.env.CLIENT_URL}/auth/verify-email/${userToken}`
    ),
    attachments: [],
  });
...
export const verifyEmail = async (req, res, _next) => {
  ...
  const user = await User.findOneAndUpdate(
    ...
    "-password -isEmailVerified -verificationCode -verificationCodeExpires -resetPasswordToken -resetPasswordExpires -favoriteRecipes -ownedIngredients"
  );

  if (!user) {
    return _next(new Error("user not found", { cause: 404 }));
  }
  res.status(200).json({
    success: true,
...
export const login = async (req, res, _next) => {
  ...
  const isCodeSent = await sendmailservice({
    ...
    message: loginVerificationEmailTemplete.replace(
      "{{code}}",
      verificationCode
    ),
    attachments: [],
...
export const verifyLoginCode = async (req, res, _next) => {
  const { code } = req.body;
...
export const refreshToken = async (req, res, _next) => {
  ...
  refreshToken = jwt.sign({ id: user._id }, process.env.JWT_SECRET_refresh, {
    expiresIn: "6d",
  });
...
export const forgetPassword = async (req, res, _next) => {
  ...
  try {
    ...
    await sendmailservice({
      ...
      message: forgetPasswordRequestEmailTemplete.replace(
        `{{reset_link}}`,
...
export const resendOtp = async (req, res, _next) => {
  const { email } = req.body;
  if (!email) {
  ...
  if (
    new Date(user.verificationCodeExpires + 1000 * 60 * 10).getTime() <
    Date.now()
  ) {
    return res
      .status(400)
      .json({ message: "you can resend otp only within 10 minutes" });
  }
  const verificationCode = generateVerificationCode();
  user.verificationCode = verificationCode;
  user.verificationCodeExpires = Date.now() + 1000 * 60 * 5;
...
export const getProfile = async function (req, res, _next) {
  const user = req.user;
  if (!user) {
    return res.status(404).json({ message: "user not found" });
  }
...
export const deleteUser = async function (req, res, _next) {
  const user = req.user;
...
export const uploadProfileImg = async (req, res, _next) => {
  const user = req.user;
  const file = req.file;
  if (!file) {
...
export const deleteProfileImg = async (req, res, _next) => {
  const user = req.user;
  if (!user.profileImage?.public_id) {
    return res.status(400).json({ message: "no profile image to delete" });
  }

  const deletedImg = await cloudinaryConfig().uploader.destroy(
...
export const toogleFavourite = async (req, res, _next) => {
  const user = req.user;
...
export const getFavouriteRecipes = async (req, res, _next) => {
  const user = req.user;
  const apiFeatures = new ApiFeatures(Recipe, req.query)
    .condition({ _id: { $in: user.favoriteRecipes } })
    .limitFields(
      "-createdAt -updatedAt -Images.URLs.public_id -Images.URLs._id -Images.customID -slug -ingredients._id -__v"
    )
    .populate([
      {
        path: "country",
      ...
      },
      {
        ...
        select:
          "name image.secure_url _id basePrice appliedPrice stock Average_rating discount",
      },
      {
        path: "createdBy",
